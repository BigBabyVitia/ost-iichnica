            ...
            return (
              <Motion.div
                ...
                className={`${styleClasses} transition-all duration-300 rounded-[24px] md:rounded-[32px] p-6 md:p-10 flex items-center justify-between group cursor-pointer ${
                  index === 0 ? "md:col-span-2 w-full" : "col-span-1"
                }`}
              >
              <div className={`text-left ${index === 0 ? "w-full pr-4 md:pr-8" : "max-w-[80%]"}`}>
                <span className="text-[#FF5833] font-normal text-[15px] md:text-lg opacity-80 mb-2 md:mb-3 block">
                  {card.category}
                </span>
                <h3 className={`text-white font-medium leading-snug ${index === 0 ? 'text-[28px] md:text-[40px] w-full md:whitespace-nowrap' : 'text-xl md:text-[28px]'}`}>
                  {card.title}
                </h3>
              </div>
